FKM.gk <- function (X, k, m, vp, RS, stand, startU, index,alpha,conv, maxit, seed = NULL)
  {
    if (missing(X))
    ...
    {
      comp = k[c]
      main.temp <- mainFKM_gk_U(data = X, m = m, index = index,
                                alpha = alpha,n = n, vp = vp[1:comp],p = p,
                                k = k[c], U = startU, conv = conv, maxit = maxit)
      if(main.temp$warn == 1)
      {
      ...
      comp = k[c]
      main.temp <- mainFKM_gk(data = X, m = m, index = index,
                              alpha = alpha,n = n,vp = vp[1:comp] ,p = p,
                              k = k[c], rs = RS, conv = conv, maxit = maxit)
      if(main.temp$warn == 1)
      {

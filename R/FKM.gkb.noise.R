FKM.gkb.noise <- function (X, k, m, vp, delta, gam, mcn, RS, stand, startU, index,alpha,conv, maxit, seed = NULL)
  {
    if (missing(X))
    ...
    if ((check!=1))
    {
      ...
      if(checkDelta)
      {
        Hd=FKM.gkb(X,k[c],m = m,RS=1,stand = stand,conv=1e-6,seed = seed)$H
        Dd = euclidean_distance(data = X,H = Hd,n = n,k = k[c])
        delta[c] <- sqrt(mean(Dd))
      }
      main.temp <- mainFKM_gkb_noise_U(data = X, m = m,delta = delta[c], index = index,alpha = alpha,
                                       n = n, p = p, k = k[c], U = startU, conv = conv,
      ...
      main.temp <- mainFKM_gkb_noise(data = X, m = m, delta = delta[c], index = index,alpha = alpha,
                                     n = n, p = p, k = k[c], rs = RS, conv = conv,
                                     maxit = maxit,gam = gam,mcn = mcn,vp = vp[1:comp])

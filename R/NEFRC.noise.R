...
NEFRC.noise = function(D, k, m, delta, RS, startU,index,alpha,conv, maxit, seed = NULL)
{
  ...
  if (delta==0)
  {
    stop("When delta=0, the standard algorithm is applied: run the function nefrc")
  }
  ...
  for(c in 1:nk)
  {
    if ((check!=1))
    {
      ...
      main.temp <- mainrnefrc_U(D = D,U = startU,delta = delta,m = m,n = n,k = k[c],index = index,alpha = alpha,conv = conv,maxit = maxit)

    }else{

      main.temp <- mainrnefrc(D = D,m = m, delta = delta, n = n,k = k[c],index = index,alpha = alpha,rs = RS,maxit = maxit,conv = conv)

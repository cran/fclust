FKM <- function (X, k, m, RS,stand, startU, index,alpha,conv, maxit, seed = NULL)
  {
   ...
   if(!is.null(seed))
    ...
    if ((check!=1))
    {
      ...
      main.temp <- mainFKM_U(data = X, m = m, n = n, p = p, k = k[c], index = index,alpha = alpha,U = startU, conv = conv, maxit = maxit)

    }else{

      main.temp <- mainFKM(data = X, m = m, n = n, p = p, k = k[c], index = index,alpha = alpha,rs = RS, conv = conv, maxit = maxit)

FKM.med <- function (X, k, m, RS, stand, startU, index,alpha,conv, maxit, seed = NULL)
  {
    if (missing(X))
    ...
    if(length(m) != 1){
      m = 1.5
      cat("The parameter of fuzziness m must be a single value: the default value m=1.5 will be used",fill=TRUE)
    ...
    for(c in 1:nk)
    {
      if ((check!=1))
      {
        ...
        main.temp <- mainFKM_med_U(data = X, m = m, n = n, p = p, k = k[c], U = startU, conv = conv, maxit = maxit,index = index,alpha = alpha)

      }else{

        main.temp <- mainFKM_med(data = X, m = m, n = n, p = p, k = k[c], rs = RS, conv = conv, maxit = maxit,index = index,alpha = alpha)
    ...
    U.opt = main$U
    H.opt = main$H
    medoid.opt = as.vector(main$medoid)

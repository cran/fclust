FKM.gkb <- function (X, k, m, vp, gam, mcn, RS, stand, startU, index,alpha,conv, maxit, seed = NULL)
  {
    if (missing(X))
    ...
    if ((check!=1))
    {
      ...
      comp = k[c]
      main.temp <- mainFKM_gkb_U(data = X, m = m, index = index,
                                 alpha = alpha,n = n, p = p, k = comp,
                                 U = startU, conv = conv, maxit = maxit,gam = gam,mcn = mcn,vp = vp[1:comp])

    }else{

      comp = k[c]
      main.temp <- mainFKM_gkb(data = X, m = m, index = index,
                               alpha = alpha,n = n, p = p, k = comp,
                               rs = RS, conv = conv, maxit = maxit,gam = gam,mcn = mcn,vp = vp[1:comp])
